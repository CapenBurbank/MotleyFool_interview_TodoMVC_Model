...
let QueryBuilder = class QueryBuilder extends Component {
                        ...
                        case 'date':
                            {
                                ...
                                if (!itemData.field && !itemData.key && itemData.value) { // tslint:disable-line
                                ...
                                if (!rule.value) {
                                    let elem = document.getElementById(parentId + '_valuekey' + i);
    ...
    renderValues(target, itemData, prevItemData, isRender, rule, tempRule, element) {
        ...
        this.selectedRule = filtObj.itemData; // tslint:disable-line
        if (isRender) {
    ...
    deleteGroupSuccessCallBack(args, target) {
        if (!args.cancel) {
            ...
            let removeString = [];
            if (this.isReact || this.isAngular) { // tslint:disable-line
    ...
    datePredicate(ruleColl, value, predicate, condition) {
        let pred;
        let startDate = this.setTime(new Date(value.getTime()), true);
        let endDate = this.setTime(value);
    ...
    getDate(value, format) {
        ...
        if (value.indexOf(':') > -1 && (value.indexOf('/') < 0 && value.indexOf(',') < 0
            && value.indexOf('-') < 0)) {
            currDate = new Date();
            currDate.setHours(parseInt(value.split(':')[0])); // tslint:disable-line
            currDate.setMinutes(parseInt(value.split(':')[1])); // tslint:disable-line
            if (value.split(':')[2]) {
                currDate.setSeconds(parseInt(value.split(':')[2])); // tslint:disable-line
            }
        }
        return currDate;
    }
