...
var Editor = /** @class */ (function () {
    ...
    Editor.prototype.addOrRemoveCommentsInBlock = function (block, isAdd) {
        if (block instanceof TableWidget) {
            for (var i = 0; i < block.childWidgets.length; i++) {
                ...
                for (var j = 0; j < row.childWidgets.length; j++) {
                    ...
                    for (var k = 0; k < cell.childWidgets.length; k++) {
                        ...
                        this.addOrRemoveCommentsInBlock(block_1, isAdd);
                    }
                }
            }
        }
        else {
            this.addOrRemoveCommentInPara(block, isAdd);
        }
    };
    Editor.prototype.addOrRemoveCommentInPara = function (block, isAdd) {
        for (var i = 0; i < block.childWidgets.length; i++) {
            ...
            for (var j = 0; j < lineWidget.children.length; j++) {
                this.addOrRemoveCommentsInline(lineWidget.children[j], isAdd);
            }
        }
    };
    Editor.prototype.addOrRemoveCommentsInline = function (inline, isAdd) {
        if (inline instanceof CommentCharacterElementBox && inline.commentType === 0 && !isNullOrUndefined(inline.comment)) {
            var comment = inline.comment.ownerComment ? (inline.comment.ownerComment) : inline.comment;
            if (isAdd) {
                ...
                if (revision.revisionType === 'Insertion') {
                    if (this.isRevisionMatched(elementBox, undefined)) {
                        ...
                        if (isNullOrUndefined(elementIndex)) {
                            ...
                            var count = 1;
                            if (revision.range.length > index_1 + 1 && revision.range[index_1 + 1] instanceof WCharacterFormat) {
                                count += 1;
                            }
                            revision.range.splice(index_1, count);
                            if (revision.range.length === 0) {
                                this.owner.revisionsInternal.remove(revision);
                            }
