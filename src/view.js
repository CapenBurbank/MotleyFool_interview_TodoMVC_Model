...
/**
 * Used for implementation of toggling priority in 6th layer of child nodes of list Item
 * 
 * @param {!Element} element 
 * 
 * @this define
 *	'dataset' accesses value of data-id of specific ListItem
 *	 
 *	
 */

const _deepItemId = element => parseInt(element.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.dataset.id, 10);
...
export default class View {
	...
	constructor(template) {
		...
		this.$priorityMarker = qs('.priorityMarker');

		this.$priorityHigh = qs('.highButton');
		this.$priorityMedium = qs('.meduimButton');
		this.$priorityLow = qs('.lowButton');

		$delegate(this.$todoList, 'li label', 'dblclick', ({target}) => {
			this.editItem(target);
		});

		$delegate(this.$todoList, '.priorityChoice', 'click', ({target})=> {
			this.toggleDropdowns(target);
		});
		
	}

	/**
	 * update items priority 1, 2, 3
	 * 
	 * @param {Function} handler
	 */

	bindTogglePriority(handler) {
		$delegate(this.$todoList, '.highButton', 'click', ({target})=> {
			handler(_deepItemId(target), 3);
		});
		$delegate(this.$todoList, '.mediumButton', 'click', ({target})=> {
			handler(_deepItemId(target), 2);
		});
		$delegate(this.$todoList, '.lowButton', 'click', ({target})=> {
			handler(_deepItemId(target), 1);
		});
	}

	/**
	 * Toggles visibility of icon dropdowns
	 * 
	 * @param {!Element} target 
	 */

	toggleDropdowns(target) {

		const iconDropdown = target.firstElementChild.nextElementSibling;

		if (!iconDropdown.classList.contains("show")) {
			iconDropdown.classList.add("show");
		}
		else if (iconDropdown.classList.contains("show")) {
	...
	/**
	 * searches document for any elements matching priority from item, conditionally renders color on controller.filter()
	 *
	 */

	editItemPriorityMarker() {
		var priorityMarker_High = document.querySelectorAll('[data-id="3"]');
		var priorityMarker_Medium = document.querySelectorAll('[data-id="2"]');
		var priorityMarker_Low = document.querySelectorAll('[data-id="1"]');

		priorityMarker_High.forEach(elem => elem.setAttribute("style", "background-color: red;"));
		priorityMarker_Medium.forEach(elem => elem.setAttribute("style", "background-color: yellow;"));
		priorityMarker_Low.forEach(elem => elem.setAttribute("style", "background-color: green;"));
	}
