...
import {emptyItemQuery, ItemList} from './item';
...
export default class Controller {
	...
	constructor(store, view) {
		...
		view.bindTogglePriority(this.editItemPriority.bind(this));
	 ...
	 *
	 * @param {string} raw '' | '#/' | '#/active' | '#/completed' | '#/priority'
	 */
	setView(raw) {
	...
	/**
	 * Edit current priority of list item (1,2,3) 3 being highest priority, 1 being lowest priority
	 * @param {!number} id
	 * @param {!number} priority
	 */

	editItemPriority(id, priority) {
		this.store.update({id, priority}, ()=> {
			this._filter(true);
		});
	}
	...
	_filter(force) {
		...
		if (force || this._lastActiveRoute !== '' || this._lastActiveRoute !== route) {
			...
			/**
			 * if route is priority, list becomes sorted and forced to re-render
			 */
			if (route == 'priority') {
				this.store.sort(this.view.showItems.bind(this.view));
			}
		}
		...
		this.view.editItemPriorityMarker();
